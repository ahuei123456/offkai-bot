...
export const STRINGS = {
  en: {
    ...
    startsInDays: (n: number) => `In ${n} day${n === 1 ? '' : 's'}`,
    startsInHours: (h: number, m: number) => `In ${h} hr${h === 1 ? '' : 's'}${m > 0 ? ` ${m} min` : ''}`,
  ...
  ja: {
    ...
    startsInDays: (n: number) => `あと${n}日`,
    startsInHours: (h: number, m: number) => `あと${h}時間${m > 0 ? `${m}分` : ''}`,
